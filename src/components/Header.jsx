  ...
  return (
    <nav className="bg-indigo-500 border-gray-200 px-2 sm:px-4 py-2.5 dark:border-r-indigo-700">
      <div className="container flex flex-wrap justify-between items-center mx-auto">
        ...
        <div className={visible ? 'w-full md:block md:w-auto' : 'hidden w-full md:block md:w-auto'} id="navbar-default">
          <ul className="flex flex-col mt-4 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium">
            {navigation.map((item) => {
              return (
                <li className="flex justify-center items-center" key={item.name}>
                  <a href={item.href} to={item.href} className="hover:bg-sky-500 block py-2 pr-4 pl-3 text-white bg-blue-700 rounded md:bg-transparent md:text-white md:p-0 dark:text-white">{item.name}</a>
                </li>
              )
            })}
            <li className="flex justify-center items-center">
              <button onClick={() => handleVisible()} id="theme-toggle" type="button" className="text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 rounded-lg text-sm p-2.5">
